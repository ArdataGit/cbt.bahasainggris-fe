...
import { Plus, Search, Edit, Trash2, Loader2, Library, BookOpen, Clock, MessageSquare } from 'lucide-react';
...
export default function ReadingCategoryListPage() {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Are you sure you want to delete this category? Readings will not be deleted, but the link will be removed.')) return;

    try {
      const response = await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/reading-categories/${id}`);
      if (response.data.success) {
  ...
  return (
    <div className="max-w-6xl mx-auto py-6 px-4">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Reading Categories</h1>
          <p className="text-sm text-gray-500 mt-1">Manage categories and timers for reading tests.</p>
        </div>
        <Link 
          ...
          <table className="w-full text-left">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                ...
                <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Readings</th>
                  ...
                  <tr key={item.id} className="hover:bg-gray-50 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end gap-2">
                        <Link 
                          href={`/dashboard/readings/categories/${item.id}/edit`}
