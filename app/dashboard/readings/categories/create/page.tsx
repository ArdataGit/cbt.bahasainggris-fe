...
import { ArrowLeft, Save, Loader2, Clock, Tag, AlignLeft, Search, Check, X, BookOpen } from 'lucide-react';
...
export default function CreateReadingCategoryPage() {
  ...
  const toggleReading = (id: number) => {
    const isSelected = formData.readingIds.includes(id);
    const newReadingIds = isSelected
      ? formData.readingIds.filter(rid => rid !== id)
      : [...formData.readingIds, id];
  ...
  const removeReading = (id: number) => {
    setFormData({
      ...formData,
      readingIds: formData.readingIds.filter(rid => rid !== id)
    });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/reading-categories`, formData);
  ...
  return (
    <div className="max-w-6xl mx-auto py-6 px-4">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Create Reading Category</h1>
          <p className="text-sm text-gray-500 mt-1">Define settings and duration for a group of readings.</p>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="divide-y divide-gray-100">
          <div className="p-6 lg:p-8 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="60"
                />
                <p className="text-[10px] text-gray-500 mt-1.5 ml-1 italic">Time limit for all readings in this category.</p>
              </div>
            </div>
            ...
            <div>
              ...
              <p className="text-[10px] text-gray-500 mt-1.5 ml-1 italic">Associate multiple reading passages with this category.</p>
            </div>
