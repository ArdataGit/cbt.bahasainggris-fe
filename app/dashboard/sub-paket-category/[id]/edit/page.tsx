...
export default function EditSubPaketCategoryPage() {
  ...
  const fetchSubCategory = async () => {
    try {
      setFetching(true);
      const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/sub-paket-categories/${id}`);
      if (response.data.success) {
        setFormData({
          subCategoryName: response.data.data.subCategoryName,
          paketCategoryId: response.data.data.paketCategoryId.toString()
        });
      }
    } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/sub-paket-categories/${id}`, formData);
  ...
  return (
    <div className="max-w-6xl mx-auto py-6 px-4 text-gray-950">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Edit Sub Paket Category</h1>
          <p className="text-sm text-gray-500 mt-1">Update sub-category name and its parent category.</p>
        </div>
      </div>
