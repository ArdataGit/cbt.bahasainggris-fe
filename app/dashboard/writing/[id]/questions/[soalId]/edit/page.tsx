...
import { ArrowLeft, Save, Loader2, Plus, X, ChevronDown, AlertCircle } from 'lucide-react';
import axios from 'axios';
import Breadcrumbs from '@/app/components/breadcrumbs';

export default function EditWritingQuestionPage({ params }: { params: Promise<{ id: string, soalId: string }> }) {
  const { id: writingId, soalId } = use(params);
  ...
  const fetchQuestion = async () => {
    try {
      setFetching(true);
      const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/soal-writing/${soalId}`);
      if (response.data.success) {
        const q = response.data.data;
        setFormData({
          writingId: q.writingId,
          jenis: q.jenis,
          question: q.question,
          answers: q.AnswerWriting.length > 0 ? q.AnswerWriting.map((a: any) => a.answer) : ['']
        });
      } else {
        throw new Error(response.data.message || 'Failed to fetch question data.');
      }
    } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/soal-writing/${soalId}`, {
  ...
  return (
    <div className="max-w-4xl mx-auto py-6 px-4 text-gray-950">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Edit Writing Question</h1>
