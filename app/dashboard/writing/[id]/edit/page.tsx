...
import { ArrowLeft, Save, Loader2, AlertCircle, Tag, Search, Check, X } from 'lucide-react';
...
export default function EditWritingPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchWriting = async () => {
    try {
      setFetching(true);
      const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/writing/${id}`);
      if (response.data.success) {
        ...
        throw new Error(response.data.message || 'Failed to fetch writing material.');
      }
    } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/writing/${id}`, jsonPayload);

      if (response.data.success) {
        router.push('/dashboard/writing');
        router.refresh();
      } else {
        throw new Error(response.data.message || 'Failed to update writing material.');
      }
    } catch (err: any) {
  ...
  if (fetching) {
    return (
      <div className="max-w-4xl mx-auto py-20 flex flex-col items-center justify-center text-gray-500">
        ...
        <p className="text-lg font-medium">Fetching writing data...</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto py-6 px-4">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900 font-serif">Edit Writing</h1>
          <p className="text-sm text-gray-500 mt-1">Update title or instructions for this task.</p>
        </div>
      </div>
