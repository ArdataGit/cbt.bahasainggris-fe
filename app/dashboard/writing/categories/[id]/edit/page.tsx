...
import { ArrowLeft, Save, Loader2, Clock, Tag, AlignLeft, AlertCircle, Search, Check, X, FileText } from 'lucide-react';
...
export default function EditWritingCategoryPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [fetching, setFetching] = useState(true);
  const [fetchingWritings, setFetchingWritings] = useState(true);
  ...
  const fetchCategory = async () => {
    try {
      setFetching(true);
      const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/writing-categories/${id}`);
      if (response.data.success) {
        ...
        setFormData({
          ...
          timer: item.timer,
          writingIds: item.writings?.map((r: any) => r.id) || []
        });
      } else {
  ...
  const toggleWriting = (writingId: number) => {
    const isSelected = formData.writingIds.includes(writingId);
    const newWritingIds = isSelected
      ? formData.writingIds.filter(wid => wid !== writingId)
      : [...formData.writingIds, writingId];
    
    setFormData({
      ...formData,
      writingIds: newWritingIds
    });
  };

  const removeWriting = (writingId: number) => {
    setFormData({
      ...formData,
      writingIds: formData.writingIds.filter(wid => wid !== writingId)
    });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/writing-categories/${id}`, formData);

      if (response.data.success) {
        router.push('/dashboard/writing/categories');
        router.refresh();
      } else {
  ...
  const selectedWritings = writings.filter(w => formData.writingIds.includes(w.id));
    ...
    <div className="max-w-6xl mx-auto py-6 px-4">
      <Breadcrumbs 
        items={[
          ...
          { label: 'Categories', href: '/dashboard/writing/categories' },
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Edit Writing Category</h1>
          ...
          <div className="p-6 lg:p-8 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="md:col-span-2">
                ...
                <input
                  ...
                  placeholder="e.g. TOEFL Writing Section"
                />
              </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                >
                  {selectedWritings.length === 0 && !isDropdownOpen && (
                    <span className="text-gray-400 text-sm">Select writings for this category...</span>
                  )}
                  {selectedWritings.map(w => (
                    ...
                    >
                      ...
                      <button 
                        type="button" 
                        onClick={(e) => { e.stopPropagation(); removeWriting(w.id); }}
                  ...
                  <div className="flex-1 min-w-[120px]">
                    <input
                      ...
                      placeholder={selectedWritings.length > 0 ? "Search more..." : ""}
                      value={searchTerm}
                      onChange={(e) => {
                    ...
                    <div className="absolute top-full left-0 w-full mt-2 bg-white border border-gray-200 rounded-xl shadow-xl z-20 max-h-60 overflow-y-auto divide-y divide-gray-50 animate-in fade-in slide-in-from-top-2 duration-200">
                      {fetchingWritings ? (
                        ...
                        <div className="p-4 text-center text-gray-500 text-sm italic">
                          No writings found for "{searchTerm}"
                        </div>
                      ) : (
                        filteredWritings.map(w => {
                          const isSelected = formData.writingIds.includes(w.id);
                          return (
                            <div 
                              key={w.id}
              ...
              <p className="text-[10px] text-gray-500 mt-1.5 ml-1 italic">You can add or remove associations between writings and this category.</p>
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Describe what kind of writings are in this category..."
              />
            </div>
          </div>
