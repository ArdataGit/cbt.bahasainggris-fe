...
export default function EditListeningQuestionPage({ params }: { params: Promise<{ id: string, soalId: string }> }) {
  const { id: listeningId, soalId } = use(params);
  ...
  const [formData, setFormData] = useState({
    ...
    question: '',
    options: [] as { id?: number; text: string; isCorrect: boolean; imageUrl?: string }[]
  });
  ...
  const fetchQuestion = async () => {
    try {
      setFetching(true);
      const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/soal-listening/${soalId}`);
      const data = response.data;
      if (data.success) {
        const q = data.data;
        setFormData({
          listeningId: q.listeingId,
          question: q.question,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/soal-listening/${soalId}`, formData);
  ...
  return (
    <div className="max-w-4xl mx-auto py-6 px-4 text-gray-950">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Edit Listening Question</h1>
