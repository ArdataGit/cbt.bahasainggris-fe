...
interface Question {
  ...
  AnswerWriting?: { id: number; answer: string }[];
...
export default function UserHistoryMeDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  const fetchUserDetail = async () => {
    try {
      ...
      const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/history/me/${id}`, {
  ...
  if (loading) {
    return (
      <div className="max-w-5xl mx-auto py-20 flex flex-col items-center justify-center text-gray-500">
        ...
        <p className="font-bold text-lg tracking-tight">Generating report...</p>
      </div>
    );
  }
  ...
  if (error || !data) {
    return (
      <div className="max-w-5xl mx-auto py-20 px-4 text-center">
        ...
        <h2 className="text-2xl font-black text-gray-900 mb-4 tracking-tighter uppercase">Access Error</h2>
        <p className="text-gray-500 mb-8 max-w-md mx-auto">{error || 'Could not find the requested data.'}</p>
        <Link 
          href="/dashboard/history-user"
          className="inline-flex items-center gap-2 px-8 py-3 bg-gray-950 text-white rounded-2xl font-bold hover:bg-gray-800 transition-all shadow-xl"
        >
          <ArrowLeft size={18} />
          Go Back
        </Link>
  ...
  const renderSection = (skill: string) => {
    ...
    if (histories.length === 0) {
      return (
        <div className="py-20 text-center bg-gray-50 rounded-[2.5rem] border border-dashed border-gray-200 text-gray-400 font-medium italic">
          No submissions for this section yet.
        </div>
      );
    }
            ...
            <div key={h.id} className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden group hover:shadow-md transition-shadow">
              ...
              {isExpanded && (
                <div className="p-8 border-t border-gray-100 animate-in slide-in-from-top-4 duration-300">
                          ...
                          <div key={q.id} className="relative pl-8 border-l-2 border-gray-100 pb-2">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-4">
                              <div className="p-3 rounded-xl border border-gray-100 bg-gray-50/50">
                                <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">Your Answer</p>
                  ...
                  {/* Writing / Speaking Response */}
                  {(skill === 'writing' || skill === 'speaking') && (
                    <div className="space-y-6">
                      {questions.map((q, qIdx) => {
                         const qAnswer = (h.soalHistories?.find(sh => (sh as any).soalWritingId === q.id || (sh as any).soalSpeakingId === q.id)?.answer || '').trim();
                           ...
                           <div key={q.id} className="mb-8 last:mb-0 relative">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 ml-8">
                                <div className={`p-4 rounded-2xl border ${isShortAnswer ? (isCorrect ? 'bg-emerald-50/10 border-emerald-100' : 'bg-red-50/10 border-red-100') : 'bg-gray-50 border-gray-100'}`}>
                                  <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-2">Your response</p>
                                ...
                                {correctAnswers.length > 0 && (
                                  <div className="p-4 bg-emerald-50/50 rounded-2xl border border-emerald-100">
                                    <p className="text-[9px] font-black text-emerald-500 uppercase tracking-widest mb-2">Model Answer</p>
                                    <div className="space-y-2">
                                      {correctAnswers.map((ca, caIdx) => (
                                        <p key={ca.id} className="text-sm font-bold text-emerald-800">{ca.answer}</p>
                                      ))}
                                    </div>
                                  </div>
                                )}
                      ...
                      {h.answer && (
                        <div className="p-6 md:p-8 bg-blue-50/30 rounded-[2rem] border border-blue-100/50 relative overflow-hidden">
                          <p className="text-[10px] font-black text-blue-400 uppercase tracking-widest mb-3">
                            {skill === 'speaking' ? 'Voice Recording Response' : 'Overall Response / Essay'}
                          </p>
                          <div className="text-gray-800 leading-relaxed text-sm whitespace-pre-wrap font-medium">
                            {skill === 'speaking' && isAudioPath(h.answer) ? (
                              ...
                              <div className="p-4 bg-white/50 rounded-2xl border border-blue-100/30 font-serif">
                                {h.answer}
                              </div>
                            )}
                          </div>
    ...
    <div className="max-w-6xl mx-auto py-6 px-4 pb-20">
      <Breadcrumbs 
        items={[
          ...
          { label: 'Attempt Detail', active: true },
        ]} 
      />
          ...
          <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-xl shadow-blue-900/5 p-8 text-center sticky top-24">
            ...
            <div className="space-y-4 text-left border-t border-gray-50 pt-8">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-xl bg-gray-50 text-gray-400 flex items-center justify-center shrink-0"><Mail size={16} /></div>
                <div className="overflow-hidden"><p className="text-[10px] font-black text-gray-300 uppercase tracking-widest mb-0.5">Email</p><p className="text-xs font-bold text-gray-600 truncate">{data.email}</p></div>
              </div>
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-xl bg-gray-50 text-gray-400 flex items-center justify-center shrink-0"><Calendar size={16} /></div>
                <div><p className="text-[10px] font-black text-gray-300 uppercase tracking-widest mb-0.5">Date</p><p className="text-xs font-bold text-gray-600">{formatDate(data.createdAt).split(' pukul ')[0]}</p></div>
              </div>
            </div>
            ...
            <div className="mt-10 p-6 bg-blue-600 rounded-[2rem] text-white shadow-xl shadow-blue-200">
               ...
               <p className="text-[9px] font-black uppercase tracking-[0.2em] opacity-60 mb-1">Overall Percentage</p>
            ...
            {tabs.map((tab) => (
              <button
                ...
                className={`flex-1 min-w-[120px] flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-xs font-black transition-all ${
                  activeTab === tab.id ? 'bg-white text-gray-900 shadow-md ring-1 ring-black/5' : 'text-gray-400 hover:text-gray-600'
                }`}
              >
                <tab.icon size={16} className={activeTab === tab.id ? `text-${tab.color}-600` : ''} />
                <span className="uppercase tracking-widest">{tab.label}</span>
                {tab.count > 0 && <span className={`w-5 h-5 rounded-md flex items-center justify-center text-[10px] ${activeTab === tab.id ? 'bg-gray-900 text-white' : 'bg-gray-200 text-gray-500'}`}>{tab.count}</span>}
              </button>
            ))}
          </div>
