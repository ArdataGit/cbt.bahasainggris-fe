...
export default function EditPaketPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        throw new Error(response.data.message || 'Failed to update paket.');
      }
    } catch (err: any) {
  ...
  if (fetchingData && !formData.name) {
      return (
        <div className="max-w-4xl mx-auto py-12 px-4 flex flex-col items-center justify-center text-gray-500">
           <Loader2 size={48} className="animate-spin text-blue-500 mb-4" />
           <p className="font-medium text-lg">Loading Exam Package Data...</p>
        </div>
      );
  }
