...
export default function CreatePaketPembelianPage() {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [fetchingPakets, setFetchingPakets] = useState(true);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/paket-pembelians`, payload);
      if (response.data.success) {
  ...
  return (
    <div className="max-w-4xl mx-auto py-6 px-4">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Buat Paket Pembelian Baru</h1>
          <p className="text-sm text-gray-500">Bundle Multiple Test Packages into a single purchase.</p>
        </div>
      </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden p-6 md:p-8">
          ...
          <div className="border-t border-gray-100 pt-8 mt-8">
            <div className="flex items-center justify-between mb-4">
               <div>
                 <h3 className="text-sm font-bold text-gray-900">Pilih Isi Paket Test</h3>
          ...
          <div className="flex items-center justify-end gap-4 mt-12 border-t border-gray-100 pt-8 px-6 -mx-6 md:-mx-8">
            ...
            >
              ...
              <span>Simpan Paket Pembelian</span>
            </button>
          </div>
        </div>
      </form>
