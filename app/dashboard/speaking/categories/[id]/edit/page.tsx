...
import { ArrowLeft, Save, Loader2, Check, X, Search, Library, AlertCircle } from 'lucide-react';
...
export default function EditSpeakingCategoryPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  const fetchCategory = async () => {
    try {
      setFetching(true);
      const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/speaking-categories/${id}`);
      if (response.data.success) {
        ...
        setFormData({
          ...
          timer: cat.timer,
          speakingIds: cat.speakings?.map((s: any) => s.id) || []
        });
      } else {
  ...
  const toggleSpeaking = (sId: number) => {
    const isSelected = formData.speakingIds.includes(sId);
    const newSpeakingIds = isSelected
      ? formData.speakingIds.filter(id => id !== sId)
      : [...formData.speakingIds, sId];
  ...
  const removeSpeaking = (sId: number) => {
    setFormData({
      ...formData,
      speakingIds: formData.speakingIds.filter(id => id !== sId)
    });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...formData,
        speakingIds: formData.speakingIds.length > 0 ? formData.speakingIds : []
      };

      const response = await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/speaking-categories/${id}`, payload);

      if (response.data.success) {
        router.push('/dashboard/speaking/categories');
        router.refresh();
      } else {
        throw new Error(response.data.message || 'Failed to update speaking category.');
      }
    } catch (err: any) {
    ...
    <div className="max-w-4xl mx-auto py-6 px-4">
      ...
      />
       ...
       <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900 font-serif">Edit Speaking Category</h1>
          <p className="text-sm text-gray-500 mt-1">Update category details and managing speaking assignments.</p>
        </div>
      </div>
