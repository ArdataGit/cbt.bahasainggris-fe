...
import { ArrowLeft, Save, Loader2, Upload, Tag, X, Check } from 'lucide-react';
...
export default function CreateSpeakingPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/speakings`, submitData, {
        headers: {
      ...
      if (response.data.success) {
        ...
        throw new Error(response.data.message || 'Failed to create speaking material.');
      }
    } catch (err: any) {
  ...
  return (
    <div className="max-w-4xl mx-auto py-6 px-4">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900 font-serif">Create Speaking</h1>
          <p className="text-sm text-gray-500 mt-1">Add a new speaking prompt or scenario.</p>
        </div>
      </div>
