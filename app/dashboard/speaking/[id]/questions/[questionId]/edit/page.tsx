...
export default function EditSoalSpeakingPage({ params }: { params: Promise<{ id: string, questionId: string }> }) {
  ...
  const fetchData = async () => {
    try {
      ...
      const [speakingRes, questionRes] = await Promise.all([
        axios.get(`${process.env.NEXT_PUBLIC_API_URL}/speakings/${id}`),
        axios.get(`${process.env.NEXT_PUBLIC_API_URL}/soal-speakings/${questionId}`)
      ]);

      if (speakingRes.data.success && questionRes.data.success) {
        setSpeaking(speakingRes.data.data);
        const soal = questionRes.data.data;
        
        setFormData({
          jenis: soal.jenis,
          question: soal.question,
          AnswerSpeaking: soal.AnswerSpeaking?.length > 0 
            ? soal.AnswerSpeaking 
            : [{ answer: '' }]
        });
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/soal-speakings/${questionId}`, payload);
  ...
  return (
    <div className="max-w-4xl mx-auto py-6 px-4">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900 font-serif">Edit Speaking Question</h1>
